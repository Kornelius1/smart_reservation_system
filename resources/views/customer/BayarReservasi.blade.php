            ...
            tombolBayar.addEventListener('click', async function (event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        // 5. Tangani Gagal (Capture error mentah 500)

                        // Ambil teks mentah (termasuk trace PHP jika APP_DEBUG=true)
                        const errorText = await response.text();

                        // Coba parsing JSON, jika gagal tampilkan error mentah
                        try {
                            const errorJson = JSON.parse(errorText);
                            alert('Gagal membuat pembayaran: ' + (errorJson.message || 'Silakan coba lagi.'));
                        } catch (e) {
                            // Jika tidak bisa di-parse sebagai JSON (yakni, error 500 mentah)
                            console.error('SERVER CRASH FATAL:', errorText);
                            alert('TERJADI CRASH DATABASE! Silakan cek konsol browser Anda (F12 > Console) untuk detail error.');
                        }
